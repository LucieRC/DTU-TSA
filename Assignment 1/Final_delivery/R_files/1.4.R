...
init <- 10 #  transient / burnin-point
p = 4 # number of parameters
lambda_list <- seq(from = 0.01, to = 1, by = 0.01) # list of different lambda
...
for (g in 1:length(lambda_list)){
  ...
  skip <- init+1
  sum_sqr_epsilon <- sum(epsilon.all[101:n]^2) # sum of squarred error 
